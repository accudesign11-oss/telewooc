...
import {
  ...
  Plug, Package, Code2, Key, Sparkles, ShieldCheck, FileCode, Server, Save
} from "lucide-react";
...
export function ApplicationMethodsControl({
  ...
  const { wordpressApp, saveWordPressApp, isSaving } = useSettings();
  ...
  useEffect(() => {
    if (wordpressApp?.app_username) setWpUsername(wordpressApp.app_username);
    if (wordpressApp?.app_password) setWpAppPassword(wordpressApp.app_password);
  }, [wordpressApp?.app_username, wordpressApp?.app_password]);
...
`;
  ...
  const handleApplyAppPass = async () => {
    ...
    try {
      // Auto-save credentials for user
      await saveWordPressApp({
        app_username: wpUsername.trim(),
      ...
      toast({
        title: "🔑 تم التطبيق عبر كلمة سر التطبيقات وحفظ البيانات للحساب بنجاح!",
        description: `تم إرسال وحقن ميزات ${tabTitle} وحفظ كلمة سر التطبيقات تلقائياً لتُستخدم مستقبلاً.`,
      });
    } catch (e: any) {
      toast({ title: "فشل الاتصال", description: e.message || "تأكد من صحة بيانات كلمة سر التطبيقات", variant: "destructive" });
    } finally {
          ...
          <TabsContent value="app_pass" className="mt-3 space-y-3">
            <div className="p-3.5 bg-amber-500/10 border border-amber-500/20 rounded-xl space-y-3">
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-1">
                <Button onClick={handleApplyAppPass} disabled={sendingAppPass} className="flex-1 bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs gap-1.5">
                ...
                <Button
                  ...
                  onClick={async () => {
                    if (!wpUsername.trim() || !wpAppPassword.trim()) {
                      toast({ title: "أدخل اسم المستخدم وكلمة سر التطبيق أولاً", variant: "destructive" });
                      return;
                    }
                    await saveWordPressApp({ app_username: wpUsername.trim(), app_password: wpAppPassword.trim() });
                  }}
                  className="border-amber-500/40 text-amber-700 dark:text-amber-300 hover:bg-amber-500/10 text-xs font-bold gap-1.5 shrink-0"
                >
                  <Save className="h-4 w-4" />
                  حفظ البيانات دائماً للحساب
                </Button>
              </div>
            </div>
          </TabsContent>
