...
`;
  ...
  const handleCleanTransients = async () => {
    ...
    try {
      ...
      toast({
        title: "🧹 تم تنظيف الذاكرة المؤقتة والـ Transients بنجاح!",
        description: "تم تحرير ذاكرة الكاش المؤقتة وتفريغ كاش قاعدة البيانات على موقعك.",
      });
    } catch (e: any) {
      toast({
        title: "فشل التنظيف",
        description: e.message || "تأكد من ضبط رابط الموقع ومفتاح الربط/كلمة سر التطبيقات أولاً في تبويب الإعدادات",
        variant: "destructive",
      });
    } finally {
      setCleaningDb(false);
