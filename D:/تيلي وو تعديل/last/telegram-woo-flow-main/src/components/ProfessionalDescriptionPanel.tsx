...
export function ProfessionalDescriptionPanel({
  ...
  const [customInstructions, setCustomInstructions] = useState(value.customInstructions || "");
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      onChange({ ...value, customInstructions: customInstructions.trim() });

      toast({
        title: "تم معالجة وتوليد الوصف بالذكاء الاصطناعي الحقيقي! 🤖✨",
        description: `تم تطبيق تعليماتك المخصصة ومراعاة الأسلوب بنجاح (${data.provider} / ${data.model}).`
      });
    } catch (e: any) {
...
.tlv-description { direction: rtl; text-align: right; font-family: 'Cairo', system-ui, -apple-system, sans-serif; color: #1e293b; line-height: 1.7; }
.tlv-service-cards, .tlv-feature-grid { display: flex; flex-wrap: wrap; gap: 10px; margin: 16px 0; padding: 0; }
.tlv-service-card, .tlv-feature-card { flex: 1; min-width: 120px; background: #f8fafc; border: 1.5px solid #e2e8f0; border-radius: 12px; padding: 12px; text-align: center; box-shadow: 0 2px 6px rgba(0,0,0,0.04); transition: all 0.3s ease; }
...
.tlv-main-title { font-size: 18px; font-weight: 800; color: #0f172a; margin: 20px 0 10px 0; border-right: 4px solid #C89B3C; padding-right: 10px; }
...
`;
    ...
    toast({
      title: "📥 تم تنزيل ملف الـ Additional CSS المخصص بنجاح!",
      description: "يمكنك لصق محتوياته في (مظهر -> تخصيص -> CSS إضافية) أو رفعه لموقعك."
    });
  };

  return (
    <Card className="border-primary/30">
      <CardContent className="p-4 space-y-4 text-right" dir="rtl">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <Label htmlFor="pro-desc-enable" className="font-semibold cursor-pointer flex items-center gap-2 text-sm">
              <Sparkles className="h-4 w-4 text-primary" />
              إنشاء وصف احترافي بالذكاء الاصطناعي ومعالجة التعليمات المخصصة
            </Label>
            <p className="text-xs text-muted-foreground mt-1">
              يولّد HTML نظيف ومربوط بالذكاء الاصطناعي الحقيقي مع دعم كامل لوسوم <code>tlv-*</code> والتعليمات المخصصة.
            </p>
          </div>
        </div>
        ...
        {value.enabled && (
          <div className="space-y-3 pt-2 border-t">
            <div>
              <Label className="mb-2 block text-sm font-medium">طريقة ونمط التوليد:</Label>
              <RadioGroup
                value={value.style}
            ...
            <div className="space-y-1.5 p-3 bg-muted/30 border rounded-xl">
              <div className="flex items-center justify-between">
                <Label className="text-xs font-bold text-foreground">
                  تعليمات مخصصة للمعالجة والوصف (Custom Instructions):
                </Label>
              ...
              <textarea
                ...
                className="w-full p-2.5 text-xs rounded-md border bg-background border-input focus:outline-none focus:ring-1 focus:ring-primary font-medium"
                rows={3}
              />
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t">
              <div className="flex items-center gap-2">
                <Button onClick={handleGenerate} disabled={isGenerating} size="sm" className="bg-primary font-bold text-xs gap-1.5 shadow-md">
                  {isGenerating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
                  توليد بالذكاء الاصطناعي الحقيقي 🤖
                </Button>
                {localHtml && (
                  <Button variant="outline" size="sm" onClick={() => setShowPreview((s) => !s)} className="text-xs">
              ...
              <Button variant="outline" size="sm" onClick={handleDownloadCustomCss} className="gap-1.5 text-xs border-amber-500/40 hover:bg-amber-500/10 font-bold">
