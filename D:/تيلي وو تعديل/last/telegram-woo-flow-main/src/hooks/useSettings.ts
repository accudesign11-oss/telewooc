...
export interface WordPressAppSettings {
  app_username: string;
  app_password: string;
  wp_url?: string;
}
...
export function useSettings() {
  ...
  const [wordpressApp, setWordpressApp] = useState<WordPressAppSettings>({
    app_username: "",
    app_password: "",
    wp_url: "",
  });
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      if (settings && settings.length > 0) {
        ...
        // WordPress App Passwords Settings profile isolated loading
        const wpRow = settings.find(s => s.key === "wp_credentials")?.value as any || {};
        const activeWp = isSubProfile ? (activeProfile?.wp_credentials || {}) : (activeProfile?.wp_credentials || wpRow);
        setWordpressApp({
          app_username: activeWp.app_username || "",
          app_password: activeWp.app_password || "",
          wp_url: activeWp.wp_url || "",
        });

        // Google Drive Settings profile isolated loading
        const driveRow = settings.find(s => s.key === "google_drive")?.value as any || {};
        const activeDrive = isSubProfile ? (activeProfile?.google_drive || {}) : (activeProfile?.google_drive || driveRow);
        setGoogleDrive({
          drive_link: activeDrive.drive_link || "",
          folder_id: activeDrive.folder_id || "",
          api_key: activeDrive.api_key || "",
          auto_embed: activeDrive.auto_embed !== false,
        });
  ...
  const saveWordPressApp = async (settings: WordPressAppSettings) => {
    setIsSaving(true);
    try {
      await saveSetting("wp_credentials", settings);
      setWordpressApp(settings);
      await updateProfileSettingsInMemoryAndDB("wp_credentials", settings);
      toast({ title: "🔑 تم حفظ كلمة سر وسلسلة بيانات التطبيقات (WP Application Passwords) بنجاح" });
    } catch (error: any) {
  ...
  const saveGoogleDrive = async (settings: GoogleDriveSettings) => {
    setIsSaving(true);
    try {
      await saveSetting("google_drive", settings);
      setGoogleDrive(settings);
      await updateProfileSettingsInMemoryAndDB("google_drive", settings);
      toast({ title: "📁 تم حفظ وتثبيت إعدادات جوجل درايف للحساب بنجاح" });
    } catch (error: any) {
      toast({ title: "خطأ في الحفظ", description: error.message, variant: "destructive" });
    } finally {
      setIsSaving(false);
